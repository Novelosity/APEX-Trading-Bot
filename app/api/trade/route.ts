...
import { TradeSchema, parseBody } from '@/lib/validation'
import type { Trade } from '@/lib/types'

const PAIRS_ATR_APPROX: Record<string, number> = {
  'BTC/USDT': 800, 'ETH/USDT': 45, 'SOL/USDT': 3.5, 'LTC/USDT': 2.5, 'BNB/USDT': 8,
  'ADA/USDT': 0.002, 'MATIC/USDT': 0.05, 'LINK/USDT': 0.5, 'AVAX/USDT': 2.5,
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const parsed = parseBody(TradeSchema, body)
    if (!parsed.ok) {
      return NextResponse.json({ success: false, error: parsed.error }, { status: 400 })
    }

    const { pair, direction, action, tradeId } = parsed.data
    ...
    const isPaper = user.paperMode === true
    ...
    if (action === 'close') {
      ...
      const trade = openTrades.find((t) => t.id === tradeId && t.userId === session.userId)
      if (!trade) {
        return NextResponse.json({ success: false, error: 'Trade not found' }, { status: 404 })
      }

      let exitPrice: number
      if (isPaper || trade.notes?.includes('[PAPER]')) {
        // For paper trades, use a simulated price (last known or approximate)
        const client = new ExchangeClient(user)
        try {
          const ticker = await client.fetchTicker(trade.pair)
          exitPrice = ticker.last
        } catch {
          exitPrice = trade.tp1Price // fallback
        }
      } else {
        const client = new ExchangeClient(user)
        const ticker = await client.fetchTicker(trade.pair)
      ...
      const updated = await updateTrade(trade.id, session.userId, {
        status: 'closed', exitPrice, pnlUsd, pnlPct, closeReason: 'manual', closedAt: new Date().toISOString(),
      })
    ...
    if (action === 'close_all') {
      ...
      for (const trade of openTrades) {
        if (trade.userId !== session.userId) continue
        let exitPrice: number
        try {
          const ticker = await client.fetchTicker(trade.pair)
          exitPrice = ticker.last
    ...
    if (action === 'open') {
      ...
      if (botState.tradingHalted) {
        return NextResponse.json(
          { success: false, error: `Trading halted: ${botState.haltReason || 'Kill switch active'}` },
          { status: 403 }
        )
      }
      ...
      // For paper mode, use a simulated balance if no exchange connected
      let availableBalance: number
      if (isPaper && !user.apiKeyEnc) {
        availableBalance = 10000 * (user.balancePct / 100) // Default $10k virtual balance
      } else {
        const balanceData = await client.fetchBalance()
        availableBalance = balanceData.free * (user.balancePct / 100)
      }
      ...
      const positionSize = calculatePositionSize(
        availableBalance, user.riskPct, entry,
      ...
      const trade: Trade = {
        ...
        pair, direction, entryPrice: entry, positionSize,
        slPrice: sl, tp1Price: tp1, tp2Price: tp2, tp3Price: tp3,
        status: 'open',
        notes: isPaper ? '[PAPER]' : undefined,
        openedAt: new Date().toISOString(),
      }

      if (!isPaper && user.execMode === 'auto') {
        if (user.tradingMode === 'futures') await client.setLeverage(pair, user.leverage)
        const order = await client.createMarketOrder(pair, direction === 'long' ? 'buy' : 'sell', positionSize / entry)
        trade.orderId = order.id
        trade.entryPrice = order.price || entry
      }

      await saveTrade(trade)
      await updateBotState(session.userId, { totalTrades: botState.totalTrades + 1, lastTradeAt: new Date().toISOString() })

      return NextResponse.json({ success: true, data: { ...trade, isPaper } })
    }
